...
let FilterNumbers= String.filter(fun c-> Char.IsNumber(c))

let lines=data.Split('\n')
let numbers = lines |> Array.map(fun s ->
    let number = FilterNumbers(s)
    printfn "%A" number
    let processednum= String.Concat( number[0] , number[number.Length - 1])
    printfn "%s" processednum
    processednum |> int
    )

printfn "%A" numbers

let answer = numbers |> Array.sum 
